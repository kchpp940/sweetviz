...
class DataframeReport:
    ...
    def show_notebook(self, w=None, h=None, scale=None, layout=None, filepath=None, file_layout=None, file_scale=None):
        ...
        try:
            ...
            # Not running inside a notebook / IPython is not installed.
            # This is expected when scripting (e.g. example_runner), where
            # the caller only cares about the ``filepath`` output branch.
            self.verbose_print(
                "(show_notebook) IPython not available – skipping in-kernel display."
            )
